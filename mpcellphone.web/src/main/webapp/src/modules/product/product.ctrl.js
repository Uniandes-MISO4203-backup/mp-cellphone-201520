            ...
            ['CrudCreator', '$scope', 'productService', 'productModel', 'cartItemService', '$location', 'authService', 'adminService', '$filter',
                function (CrudCreator, $scope, svc, model, cartItemSvc, $location, authSvc, adminService, $filter) {
                    ...
                    this.prodductInfoActions = [{
                            ...
                            class: 'warning',
                            fn: function (record) {
                                var price = $filter('currency')(record.price, 2);
                                var discount = $filter('number')(record.discount, 2);
                    ...
                    var getComments = function (id, proveedorId) {
                        svc.comments(id).then(function (data) {
                            ...
                            for (var i = 0; i < data.length; i++) {
                                if (Number(data[i].productId) === Number(id)) {
                                    ...
                                    cont++;
                                    txt += cont + ". ";
                                    if (data[i].clientId === proveedorId) {
                                        txt += " <b>( Provider ) - </b> ";
                                    }
                                    var fecha = $filter('date')(data[i].date, 'dd/MM/yyyy');
                                    txt += data[i].comment + " - " + fecha + "";
                                }
                            }
                    ...
                    this.commentActions = [{
                            ...
                            fn: function (record) {
                                ...
                                if (authSvc.getCurrentUser()) {
                                    ...
                                    getComments(record.id, record.provider.id);
                                } else {
                                    $location.path('/login');
                                }
                            },
                                ...
                                if (authSvc.getCurrentUser()) {
                                    if ($("#comment").val().length !== 0) {
                                        svc.saveComment({
                                            ...
                                            getComments($scope.prodSelComment.id, $scope.prodSelComment.provider.id);
                                            $("#comment").val("").attr("placeholder", authSvc.getCurrentUser().name + " Says: ");
                                            $("#cantidad").html("<center>" + maximoCaracteres + "</center>");
                                            $('#comment').focus();
                                        });
