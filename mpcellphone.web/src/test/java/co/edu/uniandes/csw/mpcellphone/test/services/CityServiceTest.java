...
import co.edu.uniandes.csw.mpcellphone.services.CityService;
import java.io.File;
...
public class CityServiceTest {
    ...
    public final static List<CityDTO> cityOraculo = new ArrayList<>();
    ...
    public final static String PATH = "/cities";
    ...
    public static void setUp() {
        for (int i = 0; i < 5; i++) {
            ...
            CityDTO city = factory.manufacturePojo(CityDTO.class);
            cityOraculo.add(city);
        }
    }

    @Test
    @RunAsClient
    public void t1GetCitiesService() throws IOException {
        Client cliente = ClientBuilder.newClient();
        Response response = cliente.target(URLBASE + PATH).request().get();
        String stringCity = response.readEntity(String.class);
        List<CityDTO> listCityTest = new ObjectMapper().readValue(stringCity, List.class);
        Assert.assertEquals(Ok, response.getStatus());
        Assert.assertEquals(0, listCityTest.size());
    }
}