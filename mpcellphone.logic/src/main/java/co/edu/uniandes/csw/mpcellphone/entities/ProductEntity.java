...
@NamedQueries({
    ...
    @NamedQuery(name = "Product.getCheaperProduct", query = "select u from ProductEntity u WHERE u.provider.id = :idProvider order by u.price"),
    @NamedQuery(name = "Product.getCheaperProvider", query = "select u from ProductEntity u WHERE u.cellPhone.id = :idCellPhone order by u.price"),
